...
#+begin_src elisp :mkdirp yes :tangle ./modules/UI-sway.el
  (use-package shackle
    :demand t
    :config
...
*** WEBDEV
**** Installing [[https://github.com/hrsh7th/vscode-langservers-extracted][HTML, CSS]], [[https://github.com/theia-ide/typescript-language-server][JS, TS]] LangServers, & [[https://browsersync.io/#install][BrowserSync]]
#+begin_src nix :mkdirp yes :tangle ./lsp/webdev.nix
{
  config,
  lib,
  pkgs,
...
*** Go
**** Install [[https://github.com/golang/tools/tree/master/gopls][gopls]] and [[https://go.dev/][go]]
#+begin_src nix :mkdirp yes :tangle ./lsp/go.nix
{
  config,
  lib,
  pkgs,
...
*** Haskell
**** Install [[https://github.com/haskell/haskell-language-server][Haskell Language Server]]
#+begin_src nix :mkdirp yes :tangle ./lsp/haskell.nix
{
  pkgs,
  ...
}: {
...
** Neovim
Just in case I'm in serious TTY situation
#+begin_src nix :mkdirp yes :tangle ./vim.nix
{ config, pkgs, ... }: {
  programs.neovim = {
    enable = true;
    defaultEditor = false;
...
** SQLite3
#+begin_src nix :mkdirp yes :tangle ./sqlite.nix
{
  pkgs,
  ...
}: {
...
* Now, load all the modules (and check for sway)!
